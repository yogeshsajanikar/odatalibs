...
module Routing =
    ...
    type ODataRequest(model: EdmModel, uriParser: ODataUriParser, path: ODataPath) =
        ...
        member _.parser with get() = uriParser
        member _.path with get() = path
        
    [<Literal>]
    let odataKey = "Institis-OData-05be63fe-d45f-4e5f-9afb-3213974808dd"
    
    let getODataRequestContext (ctx: HttpContext) : ODataRequest option =
        match ctx.Items.ContainsKey odataKey with
        | true -> match ctx.Items.Item odataKey with
    ...
    let setODataRequestContext (req: ODataRequest) (ctx: HttpContext) : unit =
        match ctx.Items.ContainsKey odataKey with
        | true -> ctx |> getODataRequestContext |> Option.map (fun x -> raise (ODataRequestContextAlreadySet x)) |> ignore
    ...
    let odataRoute (model: EdmModel) (next: HttpFunc) (ctx: HttpContext) =
            let path = Uri (ctx |> SubRouting.getNextPartOfPath, UriKind.Relative)            
            let parser = ODataUriParser(model, path)
            try 
                let path = parser.ParsePath()
                ctx |> setODataRequestContext (ODataRequest(model, parser, path))
                next ctx
            with
    ...
    let metadata (next: HttpFunc) (ctx: HttpContext) =
        ...
        | Some oDataReq when oDataReq.path.Count >= 1 ->
            match oDataReq.path.FirstSegment with
            | :? MetadataSegment as metaSegment ->   next ctx
            | _                                 -> skipPipeline
        | _                                           -> skipPipeline
    ...
    let metadoc (next: HttpFunc) (ctx: HttpContext) = 
        ...
        | Some oDataReq ->
            ...
            let writeMeta (output: Stream) = async {
                use xmlWriter = XmlWriter.Create(output)
                let success, _ = CsdlWriter.TryWriteCsdl(oDataReq.model, xmlWriter, CsdlTarget.OData)
                xmlWriter.Flush () 
                return success
            }
