...
module Segment =
    ...
    let private typeToKind: IEdmType option -> EdmTypeKind =
        function
    ...
    type TargetAncestry =
        ...
        member this.targetKind = typeToKind this.targetType
        member this.elementKind = typeToKind this.elementType
    ...
    type SegmentHandler() =
        ...
        member this.targetAncestry = ancestry
        ...
        override this.Handle(segment: KeySegment) = this.HandleDefault segment
        ...
        override this.Handle(segment: PropertySegment) =
            ...
            ancestry <-
                { ancestry with
                      property = Some segment.Property }
        ...
        override this.Handle(segment: CountSegment) =
            this.HandleDefault segment
            ancestry <- { ancestry with isRaw = true }
        ...
        member this.buildCanonical (root: Uri) (key: KeySegment option) =
            let canonical =
                ...
                | Some kSegment when this.targetAncestry.isEntitySet ->
                    if
                        not (List.isEmpty this.targetAncestry.canonical)
                        && List.head this.targetAncestry.canonical :? TypeSegment
                    then
                        kSegment :> ODataPathSegment
                        :: this.targetAncestry.canonical
                    else
                        this.targetAncestry.canonical
                        @ [ kSegment :> ODataPathSegment ]
                | _ -> this.targetAncestry.canonical
    ...
    let segments (path: ODataPath) =
        let handler = SegmentHandler()
        path |> Seq.iter (fun p -> p.HandleWith handler)
        handler.targetAncestry